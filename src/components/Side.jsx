    ...
    return (
        <div className={`${styles['page__side']} ${styles[pageStyle]}`}>
          ...
          <div className={`${styles['page__side--pagination']}`}> 
            <p className={`${styles[`page__side-number--${type}`]}`}>&mdash; Page {type === 'front' ? backPage-1 : backPage} &mdash;</p>
            {(type === 'back') && <button className={styles['page__side-button--left']} onClick={()=>handlePreviousPage()} title="Previous page (mouse wheel up)"><ArrowLeft></ArrowLeft></button>}
            {(type === 'front') && <button className={styles['page__side-button--right']} onClick={()=>handleNextPage()} title="Next page (mouse wheel down)"><ArrowRight></ArrowRight></button>}
          </div>
        </div>)
}
